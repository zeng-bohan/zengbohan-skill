...
> **Embedded reference:** this document is bundled inside `zengbohan-skill` and is used only for its phase-boundary and context-window rules. The slash-command names below belong to Matt Pocock's wider skill suite and are **not** installed with this skill — treat them as background reading, not available commands.
